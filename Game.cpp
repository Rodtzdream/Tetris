...
bool check_sides(int main_figure, int offsetX, int offsetY, double angle) // перевірка чи є перешкоди зліва та справа
{
...
bool game(RenderWindow& window, Music& game_music, Music& menu_music, Sound& destruction_sound, Sound& switch_sound, Sound& select_sound, Sound& gameover_sound)
{
	...
	texture.loadFromFile("resources/tetromine.jpg");
	background.loadFromFile("resources/background_game.png");
	...
	while (window.isOpen())
	{
		...
		///*   Запам'ятовування позиції фігури на полі   *///
		{
			...
			for (int i = 0; i < 4; i++)
			{
				field[a[i].x][a[i].y] = color_main; // заповнення кольором клітинки поля
				sprite_figure_next.setPosition(b[i].x * 24, b[i].y * 24);
				window.draw(sprite_figure_next); // рендер next фігури
			}
