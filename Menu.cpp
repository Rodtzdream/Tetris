...
bool menu(RenderWindow& window, Music& game_music, Music& menu_music, Sound& destruction_sound, Sound& switch_sound, Sound& select_sound, Sound& gameover_sound)
{
	...
	game_texture.loadFromFile("resources/game.png");
	settings_texture.loadFromFile("resources/settings.png");
	exit_texture.loadFromFile("resources/exit.png");
	menuBackground_texture.loadFromFile("resources/background_menu.jpg");
	if (music_on) music_square_texture.loadFromFile("resources/volume.png");
	if (!music_on) music_square_texture.loadFromFile("resources/novolume.png");
			...
			else if (IntRect(565, 620, 30, 25).contains(Mouse::getPosition(window)))
			{
				...
				if (Mouse::isButtonPressed(Mouse::Left)) // 4
				{
					if (music_on)
					{
						...
						music_square_texture.loadFromFile("resources/novolume.png");
					}
					else
					{
						music_on = true;
						...
						music_square_texture.loadFromFile("resources/volume.png");
					}
				}
			}
			else
...
bool menu_pause(RenderWindow& window, Sprite sprite_figure_main, Sprite sprite_figure_next, Sprite sprite_background, Sprite sprite_save_figure, Text highscore_text, Text score_text, Music& game_music, Music& menu_music, Sound& destruction_sound, Sound& switch_sound, Sound& select_sound, Sound& gameover_sound)
{
	...
	resume_texture.loadFromFile("resources/resume.png");
	restart_texture.loadFromFile("resources/restart.png");
	settings_texture.loadFromFile("resources/settings.png");
	main_menu_texture.loadFromFile("resources/main_menu.png");

	if (music_on) music_square_texture.loadFromFile("resources/volume.png");
	else music_square_texture.loadFromFile("resources/novolume.png");
	...
	window.clear(Color::White); // очищення усього
	window.draw(sprite_background); // рендер фону
	fill_field(window, sprite_save_figure); // заповнення поля
...
bool settings(RenderWindow& window, Music& game_music, Music& menu_music, Sound& destruction_sound, Sound& switch_sound, Sound& select_sound, Sound& gameover_sound)
{
	...
	Font font;
	if (!font.loadFromFile("resources/blacker.ttf"))
	{
		exit(0);
	}
	...
	background_texture.loadFromFile("resources/background_menu.jpg");

	mus_gen_tex.loadFromFile("resources/general_sound.png");
	mus_game_tex.loadFromFile("resources/music.png");
	mus_menu_tex.loadFromFile("resources/sound.png");
	but_back_tex.loadFromFile("resources/back.png");

	plus_tex.loadFromFile("resources/plus.png");
	minus_tex.loadFromFile("resources/minus.png");

	mixer_field_texture.loadFromFile("resources/mixer_field.png");
	mixer_texture.loadFromFile("resources/mixer.png");
	...
	if (mus_vol_gen < 0)mus_vol_gen = 0;
	else if (mus_vol_gen > 100)mus_vol_gen = 100;

	else if (mus_vol_menu < 0)mus_vol_menu = 0;
	else if (mus_vol_menu > 100)mus_vol_menu = 100;

	else if (mus_vol_game < 0)mus_vol_game = 0;
	else if (mus_vol_game > 100)mus_vol_game = 100;
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(event))
		{
			...
			if (event.mouseMove.y >= 140 && event.mouseMove.y <= 182 && event.mouseMove.x >= 197 && event.mouseMove.x <= 397 && Mouse::isButtonPressed(Mouse::Left))
			{
				...
				if (mus_vol_gen == 100) mus_gen_vol_text.setPosition(285, 36);
				else if (mus_vol_gen == 0) mus_gen_vol_text.setPosition(293, 36);
				else mus_gen_vol_text.setPosition(291, 36);
				if (music_on)
					on_music(game_music, menu_music, destruction_sound, switch_sound, select_sound, gameover_sound);
			}
			else if (event.mouseMove.y >= 285 && event.mouseMove.y <= 327 && event.mouseMove.x >= 197 && event.mouseMove.x <= 397 && Mouse::isButtonPressed(Mouse::Left))
			{
				...
				if (mus_vol_menu == 100) mus_menu_vol_text.setPosition(285, 181);
				else if (mus_vol_menu == 0) mus_menu_vol_text.setPosition(293, 181);
				else mus_menu_vol_text.setPosition(291, 181);
				if (music_on)
					on_music(game_music, menu_music, destruction_sound, switch_sound, select_sound, gameover_sound);
			}
			else if (event.mouseMove.y >= 430 && event.mouseMove.y <= 472 && event.mouseMove.x >= 197 && event.mouseMove.x <= 397 && Mouse::isButtonPressed(Mouse::Left))
			{
				...
				if (mus_vol_game == 100) mus_game_vol_text.setPosition(285, 326);
				else if (mus_vol_game == 0) mus_game_vol_text.setPosition(293, 326);
				else mus_game_vol_text.setPosition(291, 326);
				if (music_on)
					on_music(game_music, menu_music, destruction_sound, switch_sound, select_sound, gameover_sound);
			}
			...
			else if (IntRect(416, 131, 56, 56).contains(Mouse::getPosition(window)) && Mouse::isButtonPressed(Mouse::Left))
			{
				...
				if (!isButton_select)
				{
					isButton_select = true;
					mus_vol_gen += 5;
					if (mus_vol_gen > 100)mus_vol_gen = 100;
					if (mus_vol_gen == 100) mus_gen_vol_text.setPosition(285, 36);
					else mus_gen_vol_text.setPosition(291, 36);
					if (music_on)
						on_music(game_music, menu_music, destruction_sound, switch_sound, select_sound, gameover_sound);
			...
			else if (IntRect(123, 151, 58, 22).contains(Mouse::getPosition(window)) && Mouse::isButtonPressed(Mouse::Left))
			{
				...
				if (!isButton_select)
				{
					...
					mus_vol_gen -= 5;
					if (mus_vol_gen < 0)mus_vol_gen = 0;
					if (mus_vol_gen == 0) mus_gen_vol_text.setPosition(293, 36);
					else mus_gen_vol_text.setPosition(291, 36);
					if (music_on)
						on_music(game_music, menu_music, destruction_sound, switch_sound, select_sound, gameover_sound);
					break;
			...
			else if (IntRect(416, 276, 56, 56).contains(Mouse::getPosition(window)) && Mouse::isButtonPressed(Mouse::Left))
			{
				...
				if (!isButton_select)
				{
					isButton_select = true;
					mus_vol_menu += 5;
					if (mus_vol_menu > 100)mus_vol_menu = 100;
					if (mus_vol_menu == 100) mus_menu_vol_text.setPosition(285, 181);
					else mus_menu_vol_text.setPosition(291, 181);
					if (music_on)
						on_music(game_music, menu_music, destruction_sound, switch_sound, select_sound, gameover_sound);
			...
			else if (IntRect(123, 296, 58, 22).contains(Mouse::getPosition(window)) && Mouse::isButtonPressed(Mouse::Left))
			{
				...
				if (!isButton_select)
				{
					isButton_select = true;
					mus_vol_menu -= 5;
					if (mus_vol_menu < 0)mus_vol_menu = 0;
					if (mus_vol_menu == 0) mus_menu_vol_text.setPosition(293, 181);
					else mus_menu_vol_text.setPosition(291, 181);
					if (music_on)
						on_music(game_music, menu_music, destruction_sound, switch_sound, select_sound, gameover_sound);
					break;
			...
			else if (IntRect(416, 421, 56, 56).contains(Mouse::getPosition(window)) && Mouse::isButtonPressed(Mouse::Left))
			{
				...
				if (!isButton_select)
				{
					isButton_select = true;
					mus_vol_game += 5;
					if (mus_vol_game > 100)mus_vol_game = 100;
					if (mus_vol_game == 100) mus_game_vol_text.setPosition(285, 326);
					else mus_game_vol_text.setPosition(291, 326);
					if (music_on)
						on_music(game_music, menu_music, destruction_sound, switch_sound, select_sound, gameover_sound);
			...
			else if (IntRect(123, 441, 58, 22).contains(Mouse::getPosition(window)) && Mouse::isButtonPressed(Mouse::Left))
			{
				...
				if (!isButton_select)
				{
					isButton_select = true;
					mus_vol_game -= 5;
					if (mus_vol_game < 0)mus_vol_game = 0;
					if (mus_vol_game == 0) mus_game_vol_text.setPosition(293, 326);
					else mus_game_vol_text.setPosition(291, 326);
					if (music_on)
						on_music(game_music, menu_music, destruction_sound, switch_sound, select_sound, gameover_sound);
					break;
